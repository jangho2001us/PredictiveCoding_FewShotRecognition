...
def FwdPassPlus(model, LossFun, X, Y, opt):
    ...
    Loss, Acc = LossFun(vhat[-1], target=Y, n_support=opt.num_support_tr)

    # Compute gradient of loss with respect to output
    dLdy = torch.autograd.grad(Loss, vhat[-1])[0]

    return vhat, Loss, dLdy, Acc
...
def SetPCGrads(model, epsilon, X, v=None):
    ...
    for layer in range(0, DepthPlusOne - 1):
        ...
        for p in model[layer].parameters():
            dtheta = \
                torch.autograd.grad(vtemp1, p, grad_outputs=epsilon[layer + 1], allow_unused=True, retain_graph=True)[0]
            p.grad = dtheta
...
def PCInfer(opt, model, LossFun, X, Y, ErrType, eta=.1, n=20, vinit=None):
    # Fwd pass (plus return vhat and dLdy)
    vhat, Loss, dLdy, Acc = FwdPassPlus(model, LossFun, X, Y, opt)
    ...
    return vhat, Loss, dLdy, v, epsilon, Acc
