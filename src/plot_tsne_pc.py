...
def test_v2(opt, test_dataloader, model):
    '''
    Test the model trained with the prototypical learning algorithm
    '''
    device = 'cuda:0' if torch.cuda.is_available() and opt.cuda else 'cpu'
...
def main():
    ...
    options.output += '_nsVa' + str(options.num_support_val)
    ...
    # get model
    root_save_pc = "/home/jangho/data2/exp_etri_few_pc"

    # 1-shot 20-way
    name = "220511_PC_nsTr1_nqTr5_cVa20_nsVa1_nqVa15_ep100_lr0.005_eta0.5_niter20_seed2"
    ckptpath = os.path.join(root_save_pc, name, 'best_model.pth')
    print('ckpt path: ', ckptpath)

    best_state = torch.load(ckptpath)
